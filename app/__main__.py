...
def main(environ: dict | None = None, env_path=None) -> int:
    ...
    try:  # bind before the warm-up: a busy port should not cost a paid call
        ...
        return 2
    # the warm-up opens the kept-alive connection every turn will reuse (Story 1.3, F-3)
    status = jev.warm_up(cfg.key, host=cfg.jev_host, timeout=cfg.jev_timeout, client=httpd.client)
